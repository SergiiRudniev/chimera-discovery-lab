"""Numerical, evaluator-only baselines for H012."""
...
class RandomInterventionMetrics:
    """Regret of one seeded random legal action against sampled alternatives."""
...
def evaluate_legal_random_interventions(
    ...
    *,
    ...
    """Evaluate deterministic one-step random-action regret without model inputs."""
    ...
    for sample_index in range(samples):
        ...
        for candidate in range(candidates_per_sample):
            ...
            latent_action = world.sample_latent_action(action_rng)
            action = world.render_action(latent_action)
            if (
