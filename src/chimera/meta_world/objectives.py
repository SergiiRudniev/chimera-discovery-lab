"""Training objectives for numerical Meta-World transitions."""
...
def _mechanism_alignment(
    embeddings: Tensor, mechanism_ids: Tensor, domain_ids: Tensor, margin: float
) -> Tensor:
    ...
    different_domain = domain_ids[:, None] != domain_ids[None, :]
    positives = same_mechanism & different_domain & ~diagonal
    negatives = ~same_mechanism & ~diagonal
    zero = similarity.sum() * 0.0
    positive_loss = (1.0 - similarity[positives]).mean() if bool(positives.any()) else zero
    negative_loss = (
        torch.relu(similarity[negatives] - margin).mean() if bool(negatives.any()) else zero
    )
...
def meta_world_loss(
    ...
    """Return the weighted W0 loss and its auditable components."""
    ...
    effect_mask = torch.ones_like(batch.effect_targets, dtype=torch.bool)
