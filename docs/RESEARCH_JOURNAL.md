...
- **Scope:** Development seed `260950`; the 66,283,046-parameter factual-residual
  backbone trained for 600 BF16 steps on RTX 5070. Candidate evaluation used 32
  ordinary validation states. Frozen validation seeds and all model test splits
  remained sealed.
- **Backbone:** Effect NRMSE `0.890457`, four-step rollout NRMSE `0.464651`,
  no-op-state NRMSE `0.156779` and exact outcome identity residual `0.0`.
- **Candidate result:** Mean realized regret was `0.008411` for
  uncertainty-aware search, `0.007959` for mean-only search and `0.008673` for
  legal random. The registered ratios were `0.969786` versus random and
  `1.056837` versus mean-only, above the required `0.75` and `0.90`.
- **Hard guards:** Legal-action, model-budget, simulator-budget, search-replay
  and dataset-replay rates were all `1.0`; leakage findings were zero, metrics
  finite and WG4 was not revalidated.
- **Diagnosis:** Within selected candidates, predicted effect had almost no
  relationship with realized effect (Pearson `0.0331` for uncertainty-aware and
  `-0.0094` for mean-only). Predicted standard deviation averaged `0.0881`
  against realized selected effects near `0.001`, and its correlation with
  absolute error was only `0.2036`. Pointwise NLL training therefore did not
  learn the within-state action ordering required by search.
- **Decision:** Do not open H015 frozen validation, do not promote the
  checkpoint and leave `research/results/CHM-W-H015.json` as `not_run`.
- **Next action:** Replace pointwise effect regression as the search critic with
  a preregistered multi-action, within-state ranking objective generated by
  exact shared-event simulator replays.
- **Claim boundary:** Development generated-world evidence only; no real-world
  causal, creative, business, language-independence or production claim.

## 2026-07-16 - CHM-W-H015 implementation

- **Pipeline:** A state is converted into 256 legal numerical candidates by
  deterministic CEM plus a source/target/magnitude-quartile diversity archive.
  Only eight candidates per arm are executed in the simulator.
- **Output:** Frozen records contain source slot, target slot, magnitude,
  control, predicted effect, predicted standard deviation, search score,
  archive cell and realized evaluator effect. No text enters ranking.
- **Replay and budgets:** Search replay consumes cached predictions and must
  reproduce every candidate query exactly, so audit adds zero model scores.
  Actual model-score and simulator-execution counts are hard-gated.
- **GPU reproducibility:** Two postcommit RTX 5070 BF16 smoke runs produced the
  identical EMA checkpoint SHA-256 `8a57a1b50bbf8aac2ff4389a884287708e9ad7f8b51c04e06f0aacdd3a80c8ac`,
  identical validation metrics and identical 1,676,404,736-byte peak allocated
  memory.
- **Quality gate:** Ruff and mypy passed; 161 tests passed with 80.75% coverage.
- **Data:** Existing WG4 integrity evidence is reused with `revalidated: false`.
  The dataset was not validated again.
